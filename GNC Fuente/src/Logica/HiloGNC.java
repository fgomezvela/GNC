...
public class HiloGNC extends Thread {
    ...
    public HiloGNC(IGRN grnBD, IGRN grnEntrada, LeeGRN leeBD, LeeGRN leeInput, String bd, String input, String rutaComparaciones, VentanaPrincipal padre, String rutaMatrices) {
        ...
        try {
            ...
            //Proceso paralelo lectura input
            //this.leeInput.notify();
        } catch (FileNotFoundException ex) {
    ...
    public float[] calcularCoherenciaRed() throws IOException {
        ...
        padre.getLog().writeLog("This process may take several minutes\n",true);
        if (!encontrado(bd, rutaMatrices)) {
            mBD = d.distanciaFloyd(grnBD,padre);
            Volcar.volcarFichero(mBD, rutaMatrices + System.getProperty("file.separator") + bd + ".txt", bd);
        } else {               
        ...
        if (!encontrado(input, rutaMatrices)) {
            mEntrada = d.distanciaFloyd(grnEntrada,padre);
            Volcar.volcarFichero(mEntrada, rutaMatrices + System.getProperty("file.separator") + input + ".txt", input);
        } else {
    ...
    private boolean encontrado(String bd, String rutaMatrices) {
        ...
        for (int i = 0; i < lista.length; i++) {
            if (bd.compareToIgnoreCase(lista[i].split(".txt")[0]) == 0) {
                encontrado = true;
                break;
            }
        }
